...
void test3(){
    //Ignore this for now its on Right side for now
    PIDDataSet TestPara={2.8,0.1,0.25};
    PIDDataSet AngPara={2.7,0.05,0.3};
    //Tilt.set(true);
    Clamp.set(true);
...
void test4(){
    ...
    MoveEncoderPID(TestPara, 70,14, 0.5,43,true);
    IntakeBoth(-100);
    wait(2100,msec);
    IntakeBoth(0);
    MoveEncoderPID(TestPara, -80,15, 0.6,43,true);
    //Collect other 3&score
    RunRoller(100);
    MoveEncoderPID(TestPara, 100,50, 0.5,86,true);
    MoveEncoderPID(TestPara, 40,13, 0.5,87,true);
    wait(500,msec);
    ...
    MoveEncoderPID(TestPara, -100,64, 0.5,-45,true);
    Tilt.set(true);
    Clamp.set(true);
    TurnMaxTimePID(AngPara, 180,0.5, true);
    RunRoller(100);
